...
const TEST_CASE_EXTERNAL_ID = 'SGM-8'; // cambia al ID real en tu TestLink
...
async function runTest() {
    ...
    try {
        ...
        // === Paso 6: Busca patología ===
        console.log('👀 buscando patología...');

        const cssSelector = '#tabla_Patologias_filter input[type="search"]';
        await driver.wait(
            until.elementLocated(By.css(cssSelector)), 
            5000 
        );
        await driver.findElement(By.css(cssSelector)).sendKeys('cancer');

        await driver.sleep(500);

        // === PASO 7: Hacer clic en el botón de modificar "Cancer" ===
        console.log('🖱️ Localizando y forzando clic en el botón "Cancer" con JS...');

        const codigoPatologiaAEditar = 'Px00000001';

        // XPath para localizar el enlace <a> por su atributo 'cod_patologia'
        const xpathBotonEditar = `//a[@cod_patologia='${codigoPatologiaAEditar}']`;

        await driver.findElement(By.xpath(xpathBotonEditar)).click();
        ...
        await driver.findElement(By.id('nombre_patologia')).sendKeys('Cancer Tipo1 Modificado');
        await driver.sleep(500);
