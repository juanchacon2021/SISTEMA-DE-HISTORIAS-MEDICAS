...
if ($datos_reporte && $datos_examenes_r && $datos_examenes_s && $datos_antecedentes && $datos_examenes_f) {
    ...
    $pdf->Cell(5, 7, utf8_decode('CI:'), 1, 0, 'L', 0);
    $pdf->Cell(0, 7, utf8_decode($datos_reporte->cedula_historia), 1, 1, 'L', 0);

    $pdf->Cell(20, 7, utf8_decode('NOMBRES:'), 1, 0, 'L', 0);
    $pdf->Cell(0, 7, utf8_decode($datos_reporte->nombre), 1, 1, 'L', 0);
    
    $pdf->Cell(20, 7, utf8_decode('APELLIDOS:'), 1, 0, 'L', 0);
    $pdf->Cell(0, 7, utf8_decode($datos_reporte->apellido), 1, 1, 'L', 0);
    
    $pdf->SetY(-238);
    $pdf->Cell(173, 7, utf8_decode('FECHA DE NACIMIENTO:'), 1, 0, 'R', 0);
    $pdf->Cell(0, 7, utf8_decode($datos_reporte->fecha_nac), 1, 1, 'R', 0);

    $pdf->SetY(-230);
    $pdf->Cell(185, 7, utf8_decode('EDAD:'), 1, 0, 'R', 0);
    $pdf->Cell(0, 7, utf8_decode($datos_reporte->edad), 1, 1, 'R', 0);

    $pdf->SetY(-223);
    $pdf->Cell(168, 7, utf8_decode('ESTADO CIVIL:'), 1, 0, 'R', 0);
    $pdf->Cell(0, 7, utf8_decode($datos_reporte->estadocivil), 1, 1, 'R', 0);

    $pdf->Cell(20, 7, utf8_decode('TELEFONO:'), 1, 0, 'L', 0);
    $pdf->Cell(0, 7, utf8_decode($datos_reporte->telefono), 1, 1, 'L', 0);

    // $pdf->Ln(5); // Espacio antes de la línea
    // // Establecer el color de la línea
    // $pdf->SetDrawColor(226, 37, 53); // Color rojo, por ejemplo
    // // Dibujar la línea
    // $pdf->Line(10, $pdf->GetY(), 200, $pdf->GetY()); // Coordenadas de la línea
    // $pdf->Ln(5); // Espacio después de la línea

    $pdf->Cell(20, 8, utf8_decode('DIRECCION:'), 1, 0, 'L', 0);
    $pdf->Cell(0, 8, utf8_decode($datos_reporte->direccion), 1, 1, 'l', 0);

    $pdf->Cell(22, 8, utf8_decode('OCUPACION:'), 1, 0, 'L', 0);
    $pdf->Cell(0, 8, utf8_decode($datos_reporte->ocupacion), 1, 1, 'l', 0);
    
    $pdf->Cell(10, 8, utf8_decode('HDA:'), 1, 0, 'L', 0);
    $pdf->Cell(0, 8, utf8_decode($datos_reporte->hda), 1, 1, 'l', 0);

    $pdf->Cell(20, 8, utf8_decode('ALERGIAS:'), 1, 0, 'L', 0);
    $pdf->Cell(0, 8, utf8_decode($datos_reporte->alergias), 1, 1, 'l', 0);
